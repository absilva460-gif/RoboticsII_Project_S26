...
% Video saving stuff
save_video = true;
video_filename = 'pf_moving_obstacles_8_obstaclecollisionupdate+robotvelocityincrease.mp4';
video_fps = 15;
...
obs.radii = 2.5 + 1.5*rand(1,numObs);   % randomizing the radisu between 2.5 and 4.0
...
for j = 1:numObs
    ...
    while ~valid
        ...
        % stop obstacles from overlapping (kind of working with obstacle
        % function?)
        no_overlap = true;
        for m = 1:j-1
...
for k = 1:maxSteps
    ...
    % control with robot and obstacle measurement
    [v, gamma] = controller_with_moving_obstacles(x_est, goal, obs_meas, obs_vel_meas, obs.radii, pred_horizon);
    ...
    % updating robot pose measurement
...
function [v, gamma] = controller_with_moving_obstacles(x, goal, obsC, obsV, obsR, predH)
    % Goal attraction + repulsion from current and predicted obstacle positions
    p = x(1:2);
    theta = x(3);
...
function obs = update_obstacles(obs, dt, world)
    ...
    % Step 3: prevent obstacle-obstacle overlap
    buffer = 0.3;   % small extra spacing
    ...
    for i = 1:numObs-1
        for j = i+1:numObs
            ...
            if dist < minDist
                ...
                % Simple collision response: reverse both velocities
                obs.vel(:,i) = -obs.vel(:,i);
                obs.vel(:,j) = -obs.vel(:,j);
            end
        end
